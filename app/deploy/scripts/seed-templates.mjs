#!/usr/bin/env node
// Seed sacramental journey templates
...
async function main() {
  const count = await p.sacramentalJourneyTemplate.count();
  if (count > 0) {
  ...
  const sacramentMap = new Map();
  for (const tpl of TEMPLATES) {
    if (!sacramentMap.has(tpl.sacramentName)) {
      let s = await p.sacrament.findFirst({ where: { name: tpl.sacramentName } });
      if (!s) s = await p.sacrament.create({ data: { name: tpl.sacramentName, description: tpl.description } });
      sacramentMap.set(tpl.sacramentName, s.id);
    }
  }

  let created = 0;
  for (const tpl of TEMPLATES) {
    const sacramentId = sacramentMap.get(tpl.sacramentName);
    const template = await p.sacramentalJourneyTemplate.create({
      data: { name: tpl.name, description: tpl.description, sacramentId },
    });
    for (const m of tpl.milestones) {
      await p.sacramentalMilestoneTemplate.create({
