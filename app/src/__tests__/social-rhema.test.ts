...
describe("social locale share keys", () => {
  it("keeps feed share actions together with pastoral share drafts", async () => {
    ...
    expect(social.nativeShare.action).toBeTruthy();
    expect(social.nativeShare.kind.verse).toBeTruthy();
    expect(social.companion.classes.title).toBeTruthy();
    expect(social.discovery.block).toBeTruthy();
    expect(social.discovery.foryou).toBeTruthy();
  });
});
