  ...
  const ctx = {
    ...
    entities: {
      User: {
        ...
        findFirst: vi.fn(async () => (options.handleTaken ? { id: 'other' } : null)),
...
describe('updateSocialProfile', () => {
  ...
  it('normalises and saves a handle', async () => {
    ...
    const result = await updateSocialProfile({ socialHandle: '@Ana_Catequista' }, ctx as any);
...
describe('getSocialProfile', () => {
  it('returns 404 when the handle does not exist', async () => {
    const { getSocialProfile } = await import('../server/operations/socialDiscoveryOperations');
    const { ctx } = makeCtx({ profile: null });
    ctx.entities.User.findUnique = vi.fn(async () => null);

    await expect(getSocialProfile({ handle: 'ninguem' }, ctx as any)).rejects.toMatchObject({
      statusCode: 404,
    });
  });
...
describe('searchSocial', () => {
  it('returns empty for short queries without touching the database', async () => {
    const { searchSocial } = await import('../server/operations/socialDiscoveryOperations');
    const { ctx } = makeCtx();
...
describe('recordSocialWatch', () => {
  it('creates a watch and increments viewCount the first time', async () => {
    const { recordSocialWatch } = await import('../server/operations/socialOperations');
    const { ctx, createdWatches, updatedPosts } = makeCtx();

    const result = await recordSocialWatch({ postId: 'post-1', watchSeconds: 4 }, ctx as any);
  ...
  it('updates elapsed time on a repeat watch', async () => {
    ...
    const result = await recordSocialWatch({ postId: 'post-1', watchSeconds: 9 }, ctx as any);
...
describe('toggleSocialFollow counters', () => {
  it('increments follower counters when following', async () => {
    const { toggleSocialFollow } = await import('../server/operations/socialDiscoveryOperations');
    const { ctx, updatedUsers, createdFollows } = makeCtx({ existingFollow: null });

    const result = await toggleSocialFollow({ authorId: 'author-1' }, ctx as any);

    expect(result.following).toBe(true);
    expect(createdFollows).toHaveLength(1);
    expect(updatedUsers.some((row) => row.data.socialFollowersCount)).toBe(true);
  });
});
...
describe('getSocialFeed foryou ranking', () => {
  it('ranks by recommendation score and excludes watched posts', async () => {
    ...
    const result = await getSocialFeed({ sort: 'foryou', limit: 10 }, ctx as any);

    expect(feedQueries[0].where.id).toEqual({ notIn: ['seen-1'] });
    expect(result.items.map((item: { id: string }) => item.id)).toEqual(['loud', 'quiet']);
  });
});
