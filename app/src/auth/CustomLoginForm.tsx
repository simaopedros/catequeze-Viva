      ...
      <div className="space-y-3">
        ...
        <div className="relative py-1">
          ...
          <div className="relative flex justify-center text-[11px] uppercase tracking-[0.14em]">
            <span className="bg-white px-3 text-muted-foreground">{t("login_divider")}</span>
          </div>
        </div>
        ...
        {!showEmailForm ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="h-11 w-full rounded-sm shadow-none" disabled={isLoading}>
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  {t("login_loading")}
