...
import { LayoutDashboard, Users, GraduationCap, Calendar, Settings, Heart } from 'lucide-react';
...
export function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="grid h-14 items-center" style={{ gridTemplateColumns: `repeat(${visible.length}, 1fr)` }}>
        {visible.map(item => (
          <NavLink
            key={item.to}
            to={item.to}
