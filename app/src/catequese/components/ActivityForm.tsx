...
export function ActivityForm({
  ...
  const [quizQuestions, setQuizQuestions] = useState<QuizQuestion[]>(
    initialType === "QUIZ" && initialData?.questions
      ? initialData.questions
      : [],
  );
  const [openQuestionText, setOpenQuestionText] = useState(
    initialType === "OPEN_QUESTION" && initialData?.question
      ? initialData.question
      : "",
  );
  const [checklistItems, setChecklistItems] = useState<ChecklistItem[]>(
    initialType === "PARTICIPATION_CHECKLIST" && initialData?.items
      ? initialData.items
      : [],
  );
  const [reflectionPrompts, setReflectionPrompts] = useState<
    ReflectionPrompt[]
  >(
    initialType === "GUIDED_REFLECTION" && initialData?.prompts
      ? initialData.prompts
      : [],
  );
  const [reflectionGuide, setReflectionGuide] = useState(
    initialType === "GUIDED_REFLECTION" && initialData?.guide
      ? initialData.guide
      : "",
  );
  const [dynamicSteps, setDynamicSteps] = useState<DynamicStep[]>(
    initialType === "GROUP_DYNAMIC" && initialData?.steps
      ? initialData.steps
      : [],
  );
  const [familyTaskText, setFamilyTaskText] = useState(
    initialType === "FAMILY_ACTIVITY" && initialData?.task
      ? initialData.task
      : "",
  );
  const [bibleRef, setBibleRef] = useState(
    initialType === "BIBLE_READING" && initialData?.reference
      ? initialData.reference
      : "",
  );
  const [bibleQuestions, setBibleQuestions] = useState<BibleReadingQuestion[]>(
    initialType === "BIBLE_READING" && initialData?.questions
      ? initialData.questions
      : [],
  );
  ...
  const [taskRequiresUpload, setTaskRequiresUpload] = useState(
    initialType === "TASK_WITH_ATTACHMENT" && initialData?.requiresUpload
      ? initialData.requiresUpload
      : false,
  );
  const [riteText, setRiteText] = useState(
    initialType === "RITE_CELEBRATION" && initialData?.rite
      ? initialData.rite
      : "",
  );
  ...
  const buildData = () => {
    switch (type) {
      ...
      case "OPEN_QUESTION":
        return { question: openQuestionText };
      case "PARTICIPATION_CHECKLIST":
        return { items: checklistItems };
      case "GUIDED_REFLECTION":
        return { guide: reflectionGuide, prompts: reflectionPrompts };
      case "GROUP_DYNAMIC":
        return { steps: dynamicSteps };
      case "FAMILY_ACTIVITY":
        return { task: familyTaskText };
      case "BIBLE_READING":
        return { reference: bibleRef, questions: bibleQuestions };
      case "MATCHING":
        return { pairs: matchingPairs };
      case "TASK_WITH_ATTACHMENT":
        return { requiresUpload: taskRequiresUpload };
      case "RITE_CELEBRATION":
        return { rite: riteText };
      default:
        return {};
    }
  };
  ...
  const addChecklistItem = () =>
    setChecklistItems((i) => [...i, { id: uid(), text: "" }]);
  const addReflectionPrompt = () =>
    setReflectionPrompts((p) => [...p, { id: uid(), question: "" }]);
  const addDynamicStep = () =>
    setDynamicSteps((s) => [
      ...s,
      { id: uid(), instruction: "", duration: 5, materials: "" },
    ]);
  const addBibleQuestion = () =>
    setBibleQuestions((q) => [...q, { id: uid(), question: "" }]);
  const addMatchingPair = () =>
    setMatchingPairs((p) => [...p, { id: uid(), left: "", right: "" }]);
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addQuizQuestion}>
                <Plus className="h-3 w-3 mr-1" /> {t("form.add_question")}
              </Button>
            </div>
            {quizQuestions.map((q, qi) => (
              <div
                key={q.id}
                className="rounded-sm border border-border/70 bg-white p-4 space-y-3 bg-muted/20"
              >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addChecklistItem}>
                <Plus className="h-3 w-3 mr-1" /> {t("form.add_item")}
              </Button>
            </div>
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addReflectionPrompt}>
                <Plus className="h-3 w-3 mr-1" /> {t("form.add_question")}
              </Button>
            </div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setReflectionPrompts((prev) =>
                      prev.map((r) =>
                        r.id === p.id ? { ...r, question: e.target.value } : r,
                      ),
                    )
                  }
                  className="flex-1"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addDynamicStep}>
                <Plus className="h-3 w-3 mr-1" /> {t("form.add_step")}
              </Button>
            </div>
            {dynamicSteps.map((step, i) => (
              <div
                key={step.id}
                className="space-y-2 rounded-sm border border-border/70 bg-white p-3"
              >
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-overline">
                    {t("form.step", { number: i + 1 })}
                  </Badge>
                  <Button
                    size="icon"
                    variant="ghost"
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBibleQuestions((prev) =>
                      prev.map((p) =>
                        p.id === q.id ? { ...p, question: e.target.value } : p,
                      ),
                    )
                  }
                  className="flex-1"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={addMatchingPair}>
                <Plus className="h-3 w-3 mr-1" /> {t("form.add_pair")}
              </Button>
            </div>
    ...
    <div className="space-y-4 rounded-sm border border-border/70 bg-white p-5">
      ...
      <div className="border-t pt-4">
        <p className="text-xs font-medium text-muted-foreground uppercase mb-3">
          {t("form.config", {
            type: activityTypes.find((at) => at.value === type)?.label,
          })}
        </p>
        {renderSubForm()}
      </div>
