 ...
 */
export function CreditsPill({ creditsLeft, monthlyAllowance, variant, onClick, className }: CreditsPillProps) {
  const resolvedVariant = variant
    || (creditsLeft <= 0 ? 'zero' as const
      : monthlyAllowance && creditsLeft <= Math.max(1, monthlyAllowance * 0.2) ? 'low' as const
      : 'default' as const);
  ...
  return (
    ...
    >
      <Coins className={cn('h-3 w-3', resolvedVariant === 'low' && 'text-amber-500')} />
      <span>{creditsLeft} {creditsLeft === 1 ? 'crédito' : 'créditos'}</span>
      {monthlyAllowance && resolvedVariant === 'low' && (
        <span className="text-amber-500/70">/ {monthlyAllowance}</span>
      )}
    </button>
  );
}
