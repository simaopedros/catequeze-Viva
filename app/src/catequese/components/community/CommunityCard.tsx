      ...
      <div className="p-4 cursor-pointer flex items-start justify-between" onClick={() => setExpanded(!expanded)}>
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1"><Users className="h-3 w-3" />{c._count?.memberships || 0} membros</span>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t pt-3 space-y-2 text-sm animate-in fade-in slide-in-from-top-1">
          ...
          <div className="grid gap-1 sm:grid-cols-2">
            {c.coordinatorName && <div className="flex items-center gap-1 text-muted-foreground"><User className="h-3 w-3" /><span>Resp.: {c.coordinatorName}{c.coordinatorPhone ? ` (${c.coordinatorPhone})` : ''}</span></div>}
            {c.street && <div className="text-muted-foreground">{[c.street, c.number, c.neighborhood, c.complement].filter(Boolean).join(', ')}</div>}
            {c.zipCode && <div className="text-muted-foreground">CEP: {c.zipCode}</div>}
            {c.city && <div className="text-muted-foreground">{[c.city, c.state].filter(Boolean).join(' - ')}</div>}
          </div>
        </div>
      )}
    </div>
