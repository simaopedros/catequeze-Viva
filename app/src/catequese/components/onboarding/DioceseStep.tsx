  ...
  return (
    <div className="space-y-7">
      <div className="space-y-3">
        ...
        <p className="text-sm leading-relaxed text-muted-foreground">{t('diocese.subtitle')}</p>
      </div>
      ...
      <div className="max-h-64 space-y-3 overflow-y-auto">
        {!shouldShowResults && (
          <div className="border border-border/70 px-4 py-4 rounded-sm">
            <p className="text-sm font-medium text-foreground">{t('diocese.start_hint_title')}</p>
      ...
      {!showCreate ? (
        ...
        <div className="space-y-3 border border-border/70 p-4 rounded-sm">
          <Label className="text-xs font-medium">{t('diocese.new_name_label')}</Label>
          <Input
            value={newName}
          ...
          {duplicateDiocese && (
            <div className="border border-border/70 px-3 py-2 text-xs text-muted-foreground rounded-sm">
              {t('diocese.duplicate_warning')} <strong className="text-foreground">{duplicateDiocese.name}</strong>
      ...
      <div className="flex flex-col gap-3 border-t border-border/70 pt-4">
        {selected ? (
          ...
          <p className="text-sm text-muted-foreground">{t('diocese.select_hint')}</p>
        )}
        <button
          type="button"
          onClick={onSkip}
