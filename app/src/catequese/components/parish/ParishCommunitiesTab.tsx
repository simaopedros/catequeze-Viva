  ...
  return (
    <div className="space-y-4">
      ...
      {communities.length === 0 ? (
        <div className="text-center text-muted-foreground py-12"><Building2 className="mx-auto h-8 w-8 mb-2" />{tp('no_communities')}</div>
      ) : (
                ...
                <div key={c.id} className="rounded-lg border bg-card p-4 space-y-2 md:col-span-2">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={handleUpdate} disabled={savingComm || !editCommFields.name?.trim()}>
                      {savingComm ? '...' : <><Check className="mr-1 h-3 w-3" />{tp('save')}</>}
                    </Button>
                    <Button size="sm" variant="ghost" onClick={cancelEdit}><X className="mr-1 h-3 w-3" />{tp('cancel')}</Button>
                  </div>
                </div>
              );
            }
            return (
              <div key={c.id} className="rounded-lg border p-4 hover:border-primary/30 transition-colors">
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => startEdit(c)} className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title={tp('edit')}><Pencil className="h-3.5 w-3.5" /></button>
                </div>
                {(c.coordinatorName || c.phone || c.email) && (
                  <div className="mt-2 pt-2 border-t flex flex-wrap gap-2 text-xs text-muted-foreground">
                    {c.coordinatorName && <span>{tp('responsible_abbr')} {c.coordinatorName}</span>}
