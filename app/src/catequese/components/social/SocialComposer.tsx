...
import { useQuery, previewSocialShare } from "wasp/client/operations";
...
export function SocialComposer({
  ...
  useEffect(() => {
    if (initialShare) setNativeShare(initialShare);
  }, [initialShare]);

  useEffect(() => {
    if (shareApplied.current) return;
    const native = parseNativeShareSearch(searchParams);
    if (native) {
  ...
  const { data: nativePreview } = useQuery(
    previewSocialShare,
    nativeShare ?? { kind: "VERSE", sourceId: "" },
    { enabled: Boolean(nativeShare) },
  );
  ...
  const canSubmit =
    !publishing &&
    !uploading &&
    (body.trim().length > 0 || media.length > 0 || Boolean(nativeShare)) &&
  ...
  return (
    ...
    >
      ...
      {nativePreview ? (
        <SocialShareEmbed share={nativePreview as any} compact />
      ) : null}
