...
export function handlePlanLimitError(
  ...
  const t = (key: string, opts?: Record<string, string | number | null>) =>
    i18n.t(key, { ns: "billing", ...opts });
  ...
  if (message.startsWith("LIMIT:")) {
    ...
    toast({
      title: t(`upgrade_journey.${reason}.title`, {
      ...
      action: (
        <ToastAction
          altText={t(`upgrade_journey.${reason}.cta`, {
  ...
  if (
    message.startsWith("CREDITS_EXHAUSTED:") ||
    message.startsWith("CREDITS_INSUFFICIENT:")
  ) {
    const body = message.split(":").slice(1).join(":").trim();
    const t2 = (key: string, opts?: Record<string, string>) =>
      i18n.t(key, { ns: "ai", ...opts });
    toast({
