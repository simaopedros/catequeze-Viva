...
export default function CalendarPage() {
    ...
    (async () => {
      ...
      const meetingPromises = filteredClasses.map(async (cls: any) => {
        try {
          ...
          mts.forEach((m: any) => allMeetings.push({...m, name: m.title||'Encontro', type:'class', color:'#10b981', className: cls.name, classId: cls.id}));
  ...
  const firstDay = (new Date(year,month,1).getDay() + 6) % 7; // Monday-first: 0=Mon, 6=Sun
  ...
  const FILTER_OPTIONS = [
    {v:'all',l:'Todos'},
    {v:'liturgical',l:'Litúrgico'},
    {v:'parish',l:'Paroquial'},
    {v:'class',l:'Turmas'},
    {v:'sacramental',l:'Sacramental'},
  ];

  // Monday-first weekday labels
  const MON_WEEKDAYS = ['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'];
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm">{MONTHS[month]} de {year} · {monthCount} eventos</p>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <Button variant="outline" size="sm" className="h-8 text-xs px-3" onClick={()=>setCurrentDate(new Date())}>Hoje</Button>
            ...
            <Button size="sm" className="h-8 text-xs px-3" onClick={()=>{setShowForm(true);setEventDate(`${year}-${String(month+1).padStart(2,'0')}-${String(selectedDay||1).padStart(2,'0')}`);}}><Plus className="mr-1 h-3.5 w-3.5"/>Evento</Button>
          </div>
        </div>
        ...
        {/* Calendar grid — full width on mobile, 2/3 on desktop */}
        <div className="lg:grid lg:grid-cols-3 lg:gap-6">
          <div className="lg:col-span-2 rounded-xl border bg-card overflow-hidden">
            {/* Weekday headers */}
            <div className="grid grid-cols-7 bg-muted/30">
              {MON_WEEKDAYS.map((d,i)=>(
          ...
          {/* Sidebar panel — desktop always visible, mobile as slide-up sheet */}
          {/* Desktop panel */}
          ...
          <div className="lg:hidden fixed inset-0 z-50 flex items-end">
            ...
            <div className="relative z-10 w-full max-h-[70vh] overflow-y-auto rounded-t-2xl border-t bg-card shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
              <div className="sticky top-0 flex items-center justify-between p-4 border-b bg-card/95 backdrop-blur rounded-t-2xl">
                <h3 className="font-semibold text-sm flex items-center gap-2"><Calendar className="h-4 w-4"/>Dia {selectedDay} de {MONTHS[month]}</h3>
    ...
    <>
      {selectedDay && (
        <div className="rounded-xl border bg-card p-4">
          <h3 className="font-semibold flex items-center gap-2 text-sm"><Calendar className="h-4 w-4"/>Dia {selectedDay} de {MONTHS[month]}</h3>
          {dayEvents.length===0 ? (
            <div className="text-center py-6">
              <p className="text-sm text-muted-foreground">Nenhum evento neste dia.</p>
              <Button variant="outline" size="sm" className="mt-2" onClick={openNewEventForm}><Plus className="mr-1 h-3 w-3"/>Adicionar</Button>
            </div>
          ) : (
            <div className="mt-3 space-y-2">
                ...
                <div key={e.id} className="flex items-start justify-between rounded-lg border p-3 hover:bg-muted/30 transition-colors">
                  <div className="flex items-start gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <Badge variant="outline" className="mt-1 text-[10px]">{EVENT_TYPE_LABELS[e.type] || e.type}</Badge>
                    </div>
                  </div>
              ...
              <Button variant="ghost" size="sm" className="w-full text-xs" onClick={openNewEventForm}><Plus className="mr-1 h-3 w-3"/>Novo evento</Button>
            </div>
          )}
        </div>
      )}
      ...
      {showForm && (
        <div className="rounded-xl border bg-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-sm"><Plus className="inline h-4 w-4 mr-1"/>Novo evento</h3>
            <button onClick={()=>setShowForm(false)} className="text-muted-foreground hover:text-foreground p-1"><X className="h-4 w-4"/></button>
          </div>
          <input placeholder="Nome do evento *" value={name} onChange={e=>setName(e.target.value)} className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm"/>
          ...
          <input placeholder="Descrição (opcional)" value={desc} onChange={e=>setDesc(e.target.value)} className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm"/>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleCreate} disabled={!name||!eventDate}>Criar evento</Button>
