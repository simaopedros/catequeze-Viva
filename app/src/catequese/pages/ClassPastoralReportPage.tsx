...
import {
  ...
  PieChart as RPieChart, Pie, Cell, Legend, ComposedChart, Line
} from 'recharts';
...
const STATUS_COLORS: Record<string, string> = {
  ENROLLED: '#22c55e', DROPPED: '#ef4444', TRANSFERRED: '#f59e0b',
...
export default function ClassPastoralReportPage() {
  ...
  const { data, isLoading: loading } = useQuery(getClassPastoralReport, { classId: id! });
  const [rankingFilter, setRankingFilter] = useState('all');

  const filteredRanking = useMemo(() => {
    if (!data?.catechumenRanking) return [];
    if (rankingFilter === 'all') return data.catechumenRanking;
    return data.catechumenRanking.filter((r: any) => r.enrollmentStatus === rankingFilter.toUpperCase());
  }, [data, rankingFilter]);

  const meetingsBarData = useMemo(() => data?.meetingsWithAttendance?.map((m: any) => ({
    name: m.sequenceNumber ? `#${m.sequenceNumber}` : formatDate(m.date, currentLocale, { day: 'numeric', month: 'numeric' }),
  ...
  const monthlyData = useMemo(() => {
    ...
    for (const m of data.meetingsWithAttendance) {
      ...
      if (!months[key]) months[key] = { month: key, meetings: 0, presentTotal: 0, recordTotal: 0 };
      months[key].meetings++;
      months[key].presentTotal += m.presentCount + m.justifiedCount;
      months[key].recordTotal += m.presentCount + m.absentCount + m.lateCount + m.justifiedCount;
    }
    return Object.values(months).map((m: any) => ({
      ...m,
      avgAttendance: m.recordTotal > 0 ? Math.round((m.presentTotal / m.recordTotal) * 100) : 0,
        ...
        {!data.upcomingBirthdays.length ? (
          ...
          <div className="flex flex-wrap gap-2">
            {data.upcomingBirthdays.map((b: any) => (
              <Badge key={b.catechumenId} variant="outline" className="py-1.5 px-3">
                <Gift className="h-3 w-3 mr-1 text-warning" />
                {b.name} · {t('turnsAge', { age: b.age, date: formatDate(b.nextBirthday, currentLocale) })}
              </Badge>
            ))}
          </div>
        )}
        ...
        <>
          ...
          <div className="rounded-sm border border-border/70 bg-white p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={meetingsBarData} margin={{ top: 5, right: 20, left: 0, bottom: 60 }}>
                ...
                <Tooltip />
                <Bar dataKey={t('present')} stackId="a" fill={ATTENDANCE_COLORS.present} />
                <Bar dataKey={t('late')} stackId="a" fill={ATTENDANCE_COLORS.late} />
                <Bar dataKey={t('justified')} stackId="a" fill={ATTENDANCE_COLORS.justified} />
                <Bar dataKey={t('absent')} stackId="a" fill={ATTENDANCE_COLORS.absent} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="rounded-sm border border-border/70 bg-white p-6">
            ...
            {monthlyData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={monthlyData} margin={{ top: 5, right: 20, left: 0, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
          ...
          <div className="rounded-sm border border-border/70 bg-white p-6">
            ...
            {statusPieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <RPieChart>
                  <Pie data={statusPieData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value" label={({ name, percent }: any) => `${name} ${(percent * 100).toFixed(0)}%`}>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-muted/30">
                      <th className="text-left p-3 font-medium text-xs uppercase">{t('rank')}</th>
                      <th className="text-left p-3 font-medium text-xs uppercase">{tc('name')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('present')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('late')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('justified')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('absent')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('validTotal')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('rate')}</th>
                      <th className="text-center p-3 font-medium text-xs uppercase">{t('consecutiveAbsences')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {filteredRanking.map((r: any, i: number) => (
                      <tr key={r.catechumenId} className={
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-[10px] px-1.5 py-0" style={{ color: RISK_COLORS[r.riskLevel as keyof typeof RISK_COLORS] }}>
                              {t(r.riskLevel === 'ALTO' ? 'riskHigh' : r.riskLevel === 'MÉDIO' ? 'riskMedium' : 'riskLow')}
                            </Badge>
                          </div>
                        </td>
