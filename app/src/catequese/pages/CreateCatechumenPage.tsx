...
import {
  createCatechumenSchema,
  type CreateCatechumenValues,
...
export default function CreateCatechumenPage() {
  ...
  const { data: households = [], refetch: refetchHouseholds } = useQuery(
    listHouseholds,
    { take: 200 },
  );
  ...
  const onSubmit = async (values: CreateCatechumenValues) => {
    try {
      ...
      form.setError("root", {
        message: err.message || t("catechumens.create_error"),
      });
    }
  };
  ...
  if (!canManage) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-20 text-center">
        ...
        <AppDisplayTitle as="h1" className="text-xl sm:text-xl">
          {t("catechumens.access_restricted")}
        </AppDisplayTitle>
        <p className="max-w-md text-muted-foreground">
          {t("catechumens.access_restricted_desc")}
        </p>
        <Button
          variant="outline"
          className="h-10 rounded-sm"
          onClick={() => navigate("/app/catechumens")}
        >
          {t("back")}
        </Button>
      </div>
                    ...
                    {filteredHouseholds.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-1">
                        <button
                          type="button"
                          onClick={() => setShowCreateHouseholdModal(true)}
                          className="text-brand-ink underline inline-flex items-center gap-1"
                        >
