...
export default function DocumentsPage() {
  ...
  const [uploadDialog, setUploadDialog] = useState<{ catechumenId: string; docType: string } | null>(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadDocumentMultipart({
        file: selectedFile,
        name: fileName || docTypes[uploadDialog.docType as keyof typeof docTypes],
  ...
  if (loading) return <AppShell><div className="space-y-6"><div className="h-8 w-48 animate-pulse rounded bg-muted" /><SkeletonTable rows={5} /></div></AppShell>;
  ...
  return (
      <div className="space-y-6">
        ...
        {/* Upload Dialog */}
        <Dialog open={uploadDialog !== null} onOpenChange={(open) => { if (!open) closeUploadDialog(); }}>
          <DialogContent className="sm:max-w-[440px]">
            <DialogHeader>
              <DialogTitle>{tc('documents.upload_dialog_title')}</DialogTitle>
              <DialogDescription>
                {uploadingCatechumen && tc('documents.upload_dialog_desc', {
                  name: `${uploadingCatechumen.firstName || ''} ${uploadingCatechumen.lastName || ''}`.trim(),
                  type: docTypes[uploadDialog?.docType as keyof typeof docTypes] ?? uploadDialog?.docType,
                })}
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={closeUploadDialog}>{tc('cancel')}</Button>
        ...
        {catechumens.length > 0 && (
          <div className="space-y-4">
            {catechumens.map((c: any) => {
              ...
              const pendingCount = docValues.filter((d: any) => d.status === 'PENDING').length;
              const rejectedCount = docValues.filter((d: any) => d.status === 'REJECTED').length;
              const verifiedCount = docValues.filter((d: any) => d.status === 'VERIFIED').length;
              const progressPct = totalSlots > 0 ? Math.round((verifiedCount / DOC_TYPE_KEYS.length) * 100) : 0;
                ...
                <div key={c.id} className="rounded-xl border bg-card p-4 space-y-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs text-muted-foreground flex-shrink-0">
                          {tc('documents.verified_of', { verified: verifiedCount, total: DOC_TYPE_KEYS.length })}
                        </span>
                      </div>
                    </div>
                    {pendingCount > 0 && (
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {DOC_TYPE_KEYS.map(type => {
                      ...
                      const status: 'VERIFIED' | 'PENDING' | 'REJECTED' | 'MISSING' = doc?.status || 'MISSING';

                      const statusVisual = {
                        VERIFIED: { Icon: CheckCircle, color: 'text-success', badge: 'success' as const },
                        PENDING: { Icon: Clock, color: 'text-warning', badge: 'warning' as const },
                        REJECTED: { Icon: XCircle, color: 'text-destructive', badge: 'destructive' as const },
                        MISSING: { Icon: AlertTriangle, color: 'text-muted-foreground', badge: 'secondary' as const },
                      }[status];
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            {status !== 'MISSING' && (
                              <Badge variant={statusVisual.badge} size="sm" className="flex-shrink-0">
                                {tc(`documents.status_${status.toLowerCase()}` as any)}
                              </Badge>
                            )}
                            {status === 'MISSING' && (
                          ...
                          <div className="flex items-center gap-0.5 flex-shrink-0 ml-2">
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    variant="ghost"
                                    className="h-7 w-7"
                                    onClick={() => setUploadDialog({ catechumenId: c.id, docType: type })}
                                  >
                                    <Upload className="h-3.5 w-3.5 text-muted-foreground" />
                                  </Button>
                                </TooltipTrigger>
                                <TooltipContent>{tc('documents.tooltip_upload')}</TooltipContent>
                              </Tooltip>
                            )}
                            {status === 'PENDING' && isCoordinator && (
                              <>
                                <Tooltip>
                                  ...
                                  <TooltipContent>{tc('documents.tooltip_approve')}</TooltipContent>
                                </Tooltip>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button size="icon" variant="ghost" className="h-7 w-7"
                                  ...
                                  <TooltipContent>{tc('documents.tooltip_reject')}</TooltipContent>
                                </Tooltip>
                              </>
                            )}
                            ...
                            {(status === 'VERIFIED' || status === 'REJECTED') && canUpload && (
                              <Tooltip>
                                ...
                                <TooltipContent>{tc('documents.tooltip_replace')}</TooltipContent>
                              </Tooltip>
                            )}
                            {doc && canUpload && (
                              <Tooltip>
                                ...
                                <TooltipContent>{tc('documents.tooltip_delete')}</TooltipContent>
                              </Tooltip>
                            )}
                          </div>
                        </div>
