...
function getGuardianDisplayName(g: any): string {
  if (!g) return "";
  if (g.user)
    return `${g.user.firstName || ""} ${g.user.lastName || ""}`.trim();
...
export default function FamilyDetailPage() {
  ...
  const relationshipOptions = useMemo(
    () =>
      RELATIONSHIP_KEYS.map((r) => ({
        value: r.value,
        label: t(`families.relationships.${r.key}`),
      })),
    [t],
  );
  ...
  const [confirmRemoveCatechumenOpen, setConfirmRemoveCatechumenOpen] =
    useState(false);
  const [removingCatechumenId, setRemovingCatechumenId] = useState<
    string | null
  >(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: `${t("error")}: ${e.message || t("families.error_save")}`,
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleToggleConsent = async (consentType: string, granted: boolean) => {
    ...
    try {
      ...
      toast({
        title: granted
          ? t("families.consent_granted")
          : t("families.consent_revoked"),
      });
    } catch (e: any) {
      toast({
        title: `${t("error")}: ${e.message || t("families.error_save")}`,
  ...
  const handleAddGuardian = async () => {
    ...
    try {
      ...
      toast({
        title: `${t("error")}: ${
          e.message || t("families.error_add_guardian")
        }`,
      });
    } finally {
  ...
  const handleUpdateGuardian = async () => {
    ...
    try {
      ...
      toast({
        title: `${t("error")}: ${
          e.message || t("families.error_update_guardian")
        }`,
      });
    } finally {
  ...
  const handleRemoveGuardian = async () => {
    ...
    try {
      await removeGuardianFromHousehold({
        guardianProfileId: removingGuardian.id,
      });
      ...
      toast({
        title: `${t("error")}: ${
          e.message || t("families.error_remove_guardian")
        }`,
      });
    } finally {
  ...
  const handleLinkCatechumen = async () => {
    ...
    try {
      ...
      toast({
        title:
          t("families.catechumen_linked_success") ||
          "Catequizando adicionado com sucesso!",
      });
      ...
      toast({
        title: `${t("error")}: ${
          e.message || t("families.error_link_catechumen")
        }`,
      });
    } finally {
  ...
  const handleRemoveCatechumen = async () => {
    ...
    try {
      ...
      toast({
        title:
          t("families.catechumen_removed_success") ||
          "Catequizando removido com sucesso",
      });
      ...
      toast({
        title: `${t("error")}: ${
          e.message || t("families.error_remove_catechumen")
        }`,
      });
    } finally {
  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-[460px]">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("families.relationship_placeholder")}
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!guardianFirstName.trim() || savingGuardian}
            >
              {savingGuardian && (
      ...
      <Dialog
        open={editGuardianDialogOpen}
        onOpenChange={setEditGuardianDialogOpen}
      >
        <DialogContent className="sm:max-w-[460px]">
          <DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={setEditGuardianRelationship}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("families.relationship_placeholder")}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setEditGuardianDialogOpen(false)}
              disabled={savingEditGuardian}
            >
              {t("cancel")}
            </Button>
            <Button
      ...
      <Dialog
        open={addCatechumenDialogOpen}
        onOpenChange={setAddCatechumenDialogOpen}
      >
        <DialogContent className="sm:max-w-[460px]">
          <DialogHeader>
            <DialogTitle>
              {t("families.add_catechumen") || "Adicionar Catequizando"}
            </DialogTitle>
            <DialogDescription>
              {t("families.add_catechumen_desc") ||
                "Selecione um catequizando para vincular a esta família."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <label className="text-sm font-medium">
                {t("catechumens.title") || "Catequizando"}
              </label>
              <Select
                value={selectedCatechumenId}
                onValueChange={setSelectedCatechumenId}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={
                      t("families.select_catechumen_placeholder") ||
                      "Selecione um catequizando"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setAddCatechumenDialogOpen(false)}
              disabled={linkingCatechumen}
            >
              {t("cancel")}
            </Button>
            <Button
              onClick={handleLinkCatechumen}
              disabled={!selectedCatechumenId || linkingCatechumen}
            >
              {linkingCatechumen && (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              )}
      ...
      <ConfirmDialog
        ...
        title={
          t("families.remove_catechumen_title") || "Desvincular Catequizando"
        }
        description={
          t("families.remove_catechumen_desc") ||
          "Tem certeza que deseja remover este catequizando desta família?"
        }
