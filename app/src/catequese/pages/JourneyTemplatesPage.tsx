...
export default function JourneyTemplatesPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast({
        title: t("templates.error_create"),
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: t("templates.error_update"),
        description: e.message,
        variant: "destructive",
      });
    }
  ...
  const handleDeleteMilestone = async (milestoneId: string) => {
    ...
    try {
      ...
      toast({
        title: t("templates.error_remove_milestone"),
        description: e.message,
        variant: "destructive",
      });
    }
  ...
  const handleSaveMilestone = async () => {
    ...
    try {
      await updateMilestoneTemplate({
        ...
        daysBeforeSacrament: milestoneEdit.daysBeforeSacrament
          ? parseInt(milestoneEdit.daysBeforeSacrament, 10)
          : null,
      });
      ...
      toast({
        title: t("templates.error_update_milestone"),
        description: e.message,
        variant: "destructive",
      });
    }
  ...
  const handleCopy = async (templateId: string) => {
    ...
    try {
      ...
      toast({
        title: t("templates.error_copy"),
        description: e.message || t("templates.error_restart"),
        variant: "destructive",
      });
    } finally {
      setCopyingId(null);
  ...
  const handlePublish = async (templateId: string) => {
    ...
    try {
      ...
      toast({
        title: t("templates.error_publish"),
        description: e.message || t("templates.error_restart"),
        variant: "destructive",
      });
    } finally {
          ...
          {templates.map((tmpl: any) => {
            ...
            const scopeLabel = !tmpl.parishId
              ...
              : tmpl.parish?.type === "PERSONAL"
                ...
                : t("templates.scope_parish", {
                    name: tmpl.parish?.name || "—",
                  });

            return (
              <div
                key={tmpl.id}
                className="rounded-sm border border-border/70 bg-white overflow-hidden"
              >
                    ...
                    {milestones.length === 0 ? (
                      ...
                      milestones.map((m: any) => {
                        ...
                        return (
                          <div
                            key={m.id}
                            className="flex items-start gap-3 rounded-sm border border-border/70 bg-white p-3"
                          >
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex items-center gap-3 text-xs flex-wrap">
                                    <label className="flex items-center gap-1">
                                      <input
                                        type="checkbox"
                                        checked={milestoneEdit.required}
                                        onChange={(e) =>
                                          setMilestoneEdit((prev) => ({
                                            ...prev,
                                    ...
                                    <label className="flex items-center gap-1">
                                      <input
                                        type="checkbox"
                                        checked={milestoneEdit.evidenceRequired}
                                        onChange={(e) =>
                                          setMilestoneEdit((prev) => ({
                                            ...prev,
                                    ...
                                    <label className="flex items-center gap-1">
                                      ...
                                      <input
                                        type="number"
                                        value={
                                          milestoneEdit.daysBeforeSacrament
                                        }
                                        onChange={(e) =>
                                          setMilestoneEdit((prev) => ({
                                            ...prev,
                                            daysBeforeSacrament: e.target.value,
                                          }))
                                        }
                                ...
                                <div className="flex items-center justify-between gap-2">
                                  <div>
                                    <p className="flex flex-wrap items-center gap-1.5 text-sm font-semibold tracking-tight text-brand-ink">
                                      ...
                                      {m.daysBeforeSacrament != null && (
                                        <Badge
                                          variant="outline"
                                          className="text-overline"
                                        >
                                          {t("templates.days_before_badge", {
                                            count: m.daysBeforeSacrament,
                                          })}
                                        </Badge>
                                      )}
                                    </p>
                                        ...
                                        onClick={() => {
                                          ...
                                          setMilestoneEdit({
                                            ...
                                            evidenceRequired:
                                              m.evidenceRequired,
                                            daysBeforeSacrament:
                                              m.daysBeforeSacrament?.toString() ||
                                              "",
                                          });
                                        }}
                                      >
