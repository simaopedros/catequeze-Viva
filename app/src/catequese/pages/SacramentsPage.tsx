...
export default function SacramentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeParishId) {
      result = result.filter((j: any) => {
        const enrollmentParishes =
          j.catechumenProfile?.enrollments?.map(
            (e: any) => e.class?.parishId,
          ) || [];
        return (
          enrollmentParishes.includes(activeParishId) ||
          j.catechumenProfile?.parishId === activeParishId
        );
      });
    }
    ...
    if (search) {
      result = result.filter((j: any) =>
        `${j.catechumenProfile?.firstName} ${j.catechumenProfile?.lastName}`
          .toLowerCase()
  ...
  const handleCreateJourney = async () => {
    ...
    setSaving(true);
    try {
      const templateId = templates?.find((t: any) => t.name === templateName)
        ?.id;
      if (!templateId) throw new Error(t("page.template_not_found"));
      await createSacramentalJourney({
        catechumenProfileId: selectedCatechumenId,
        templateId,
      });
      setShowForm(false);
  ...
  const statusCounts = useMemo(() => {
    ...
    return {
      ...
      blocked: all.filter(
        (j: any) => j.milestones?.some((m: any) => m.status === "REJECTED"),
      ).length,
      waitingApproval: all.filter(
        (j: any) =>
          j.milestones?.some((m: any) => m.status === "WAITING_APPROVAL"),
      ).length,
    };
  ...
  return (
    <div className="space-y-6">
      <AppPageHeader
        ...
        subtitle={
          isCatechist
            ? t("page.subtitle_catechist", { count: filtered.length })
            : t("page.subtitle_default", { count: filtered.length })
        }
        actions={
      ...
      {showForm && (
        <div className="rounded-sm border border-border/70 bg-white p-4 flex flex-col sm:flex-row gap-3">
          <Select
            value={selectedCatechumenId || "none"}
            onValueChange={(v) =>
              setSelectedCatechumenId(v === "none" ? "" : v)
            }
          >
            ...
            <SelectContent>
              <SelectItem value="none">
                {t("page.select_catechumen")}
              </SelectItem>
              {filteredCatechumens.map((c: any) => (
          ...
          <Button
            ...
            disabled={!selectedCatechumenId || saving}
          >
            {saving ? t("page.creating") : t("page.start")}
          </Button>
          <Button
            size="sm"
            variant="outline"
        ...
        <EmptyState
          ...
          title={
            search || activeFilter !== "all"
              ? t("page.empty_no_results")
              : t("page.empty_no_journey")
          }
          description={
            search ? t("page.empty_try_terms") : t("page.empty_start_desc")
          }
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {filtered.map((j: any) => {
            ...
            const hasBlocked = milestones.some(
              (m: any) => m.status === "REJECTED",
            );
            const hasWaiting = milestones.some(
            ...
            return (
              <div
                key={j.id}
                className="block rounded-sm border border-border/70 bg-white p-5 hover: transition-shadow cursor-pointer"
                onClick={() => navigate(`/app/sacramental-journeys/${j.id}`)}
              >
                  ...
                  {editingTargetDate[j.id] !== undefined ? (
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        onClick={(e) => handleSaveTargetDate(j.id, e)}
                      >
                        ✓
                      </Button>
                    </div>
                ...
                >
                      ...
                      {isCoordinator &&
                        (m.status === "COMPLETED" ||
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-5 text-overline text-muted-foreground hover:text-destructive px-1 flex-shrink-0"
                  ...
                  {milestones.length > 3 && (
                    <p className="text-overline text-muted-foreground text-center pt-1">
                      +
                      {t("page.more_milestones", {
                        count: milestones.length - 3,
                      })}
                    </p>
                  )}
                </div>
