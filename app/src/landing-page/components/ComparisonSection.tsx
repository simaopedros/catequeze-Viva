    ...
    <section className="border-y bg-muted/20">
      <div className="mx-auto max-w-5xl px-4 py-16 md:py-20">
          ...
          <table className="w-full min-w-[560px] text-left text-sm">
            ...
            <tbody>
              {rowList.map((row, index) => (
                <ComparisonRowItem key={row.criterion} row={row} delay={index * 40} />
              ))}
            </tbody>
          </table>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-muted-foreground">{tr("comparison.helper")}</p>
        </div>
      </div>
    </section>
  );
...
function ComparisonRowItem({ row, delay }: { row: ComparisonRow; delay: number }) {
    ...
    <tr ref={ref as any} className={`border-b last:border-0 ${className}`}>
      ...
      <td className="px-4 py-3.5 align-top text-muted-foreground">
        <span className="inline-flex items-start gap-2">
          <X className="h-4 w-4 text-destructive/70 shrink-0 mt-0.5" aria-hidden />
          <span>{row.old}</span>
        </span>
      </td>
