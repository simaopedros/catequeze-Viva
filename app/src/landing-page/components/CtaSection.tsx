...
export function CtaSection({ ns = "landing", responsiveCtas = false }: { ns?: string; responsiveCtas?: boolean; }) {
  ...
  return (
    <section className="max-w-3xl mx-auto px-4 pb-20">
      <div ref={ref} className={cn("rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-accent/10 border-2 border-primary/20 p-12 space-y-5 text-center relative overflow-hidden", responsiveCtas && "rounded-2xl px-5 py-8 sm:rounded-3xl sm:p-12", className)}>
        <div className="absolute top-0 right-0 w-40 h-40 bg-primary/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative space-y-5">
          <Badge variant="brand" className="inline-flex items-center gap-1.5"><BrandMark className="h-3.5 w-3.5" />{t("cta_badge")}</Badge>
