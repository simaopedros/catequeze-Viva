...
export function FamilyPortalMock({ ns = "landing" }: { ns?: string }) {
  ...
  return (
    <div className="h-full overflow-hidden p-3 sm:p-4 space-y-3 bg-background text-overline sm:text-xs">
      ...
      <div className="rounded-sm border border-brand-ink/20 bg-muted/30 p-2 flex items-start gap-2">
        <Bell className="h-3.5 w-3.5 text-brand-ink flex-shrink-0 mt-0.5" />
        <div>
      ...
      <div className="rounded-sm border border-border/70 bg-white p-2 space-y-2">
        <p className="font-brand-display flex items-center gap-1 font-semibold tracking-tight text-brand-ink">
