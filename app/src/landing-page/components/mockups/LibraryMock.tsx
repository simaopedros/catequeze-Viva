...
export function LibraryMock({ ns = 'landing' }: { ns?: string }) {
 ...
 tab.active ? 'bg-white text-[#071A2D]' : 'text-muted-foreground'
      ...
      <div className="rounded-sm border border-border/70 bg-white p-3 space-y-2">
        ...
        <div className="flex flex-wrap gap-1 pt-1">
          {[t('mockup_library.cic_ref'), t('mockup_library.dir_ref'), t('mockup_library.plan_ref')].map((ref) => (
            <span key={ref} className="rounded-sm bg-[#071A2D]/08 text-[#071A2D] px-2 py-0.5 text-overline">
              {ref}
            </span>
          ))}
        </div>
      ...
      <div className="rounded-sm border border-border/70 bg-white p-2">
        <p className="mb-1 font-semibold tracking-tight text-[#071A2D]">{t('mockup_library.plan_title')}</p>
        <div className="flex items-center gap-2">
          <span className="rounded-sm border border-border/70 bg-muted/40 px-2 py-0.5 text-overline font-semibold tracking-tight text-[#071A2D]">{t('mockup_library.published')}</span>
