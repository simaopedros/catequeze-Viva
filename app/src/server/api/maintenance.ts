...
export function maintenanceSecretMatches(provided: string, expected: string): boolean {
 ...
 */
export async function maintenanceHandler(req: Request, res: Response, context: any) {
  ...
  const tasks: MaintenanceTask[] = [
    { name: 'aiCreditsReset', run: () => resetAiCreditsJob(undefined, context) },
  ...
  try {
    for (const task of tasks) {
      ...
      try {
        ...
        logger.error('[maintenance] Task failed', { task: task.name, error: message });
        results.push({
          name: task.name,
          status: 'error',
          durationMs: Date.now() - taskStartedAt,
