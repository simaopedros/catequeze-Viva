 ...
 * - GET  /api/social/media/:id     public image delivery (fallback when no pull zone)
...
export const socialImageUploadMiddleware: MiddlewareConfigFn = (middlewareConfig) => {
...
/**
 * Public delivery for feed images. Only serves media attached to a visible
 * post, or media the uploader is still composing with.
 */
export async function serveSocialMedia(req: Request, res: Response, context: any) {
  try {
    ...
    if (!media || media.kind !== 'IMAGE' || !media.storageKey) {
      return res.status(404).json({ error: 'Mídia não encontrada.' });
    }
    ...
    res.setHeader('Content-Type', media.mimeType || stored.contentType);
    res.setHeader(
      'Cache-Control',
...
export const serveSocialMediaMiddleware: MiddlewareConfigFn = (middlewareConfig) => {
 ...
 */
export async function bunnyStreamWebhook(req: Request, res: Response, context: any) {
  ...
  await syncSocialVideoFromBunny(context, { id: media.id, bunnyVideoId: videoId });
  return res.json({ status: 'ok' });
}

export const bunnyStreamWebhookMiddleware: MiddlewareConfigFn = (middlewareConfig) => {
...
export async function syncSocialVideoFromBunny(
  ...
  if (!video) {
    await context.entities.SocialMedia.update({
      where: { id: media.id },
      data: { status: 'FAILED', failureReason: 'Vídeo não encontrado no Bunny Stream.' },
    });
    return 'FAILED';
  }
  ...
  await context.entities.SocialMedia.update({
    ...
    data: {
      ...
      thumbnailUrl:
        buildBunnyThumbnailUrl(media.bunnyVideoId, video.thumbnailFileName) ?? undefined,
      failureReason: status === 'FAILED' ? 'Falha na codificação do vídeo.' : null,
    },
  });
