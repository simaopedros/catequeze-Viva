...
import { UPLOADS_DIR, generateUploadFileName, resolveUploadFilePath } from '../uploads/helpers';
import {
  bunnyDeleteObject,
  bunnyGetObject,
  bunnyPutObject,
...
export async function storeSocialImage(params: {
  ...
  const fileName = generateUploadFileName(params.mimeType, ['jpg', 'jpeg', 'png', 'webp']);
...
export async function readSocialImage(
  ...
  return { buffer: fs.readFileSync(filePath), contentType: 'application/octet-stream' };
}
 ...
 */
export function buildSocialImageUrl(mediaId: string, storageKey: string | null): string {
