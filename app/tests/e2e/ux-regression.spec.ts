...
test.describe('UX — attendance ≤2 taps (mobile catechist)', () => {
  ...
  test('from home: reach attendance in ≤2 purposeful taps', async ({ page }) => {
    await login(page, USERS.leadCatechist.email);
    await enterFirstWorkspace(page);
    await openAuthed(page, '/app');
    await dismissCookieBanner(page);
    ...
    const checklistAttendance = page.locator('main a[href*="/attendance"]').first();
    ...
    if (
      ...
      const classesNav = mobileBottomNav(page).locator('a[href="/app/classes"]');
  ...
  test('direct deep-link attendance sheet loads on mobile', async ({ page }) => {
    await login(page, USERS.leadCatechist.email);
    await enterFirstWorkspace(page);
    await openAuthed(page, `/app/classes/${CLASS_CRISMA}/attendance`);
    await expect(page).toHaveURL(new RegExp(`/classes/${CLASS_CRISMA}/attendance`));
    await assertNoHorizontalOverflow(page);
    await assertPrimaryTouchTargets(page, { sample: 10, min: 36 });
  });
});
...
test.describe('UX — classes primary action', () => {
  ...
  test('classes page exposes primary create or list content', async ({
    ...
    const hasPrimary = await primary.isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('UX — calendar mobile agenda', () => {
  ...
  test('calendar loads and stays within viewport width', async ({ page }) => {
    ...
    await expect(
      page.locator('main').getByText(/calendário|agenda|hoje|today|evento/i).first(),
    ).toBeVisible({ timeout: 20000 });
    await assertNoHorizontalOverflow(page);
  });
});
...
test.describe('UX — families / team / reports mobile', () => {
  ...
  test('families list: primary CTA + no overflow', async ({ page }) => {
    ...
    const create = page
      ...
      .first();
    const hasCreate = await create.isVisible({ timeout: 5000 }).catch(() => false);
    const body = await page.locator('main').innerText();
...
test.describe('UX — form dirty leave guard', () => {
  ...
  test('create class shows leave dialog when dirty', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /cancelar|cancel|voltar|back/i }).first().click();

    await expect(
      page.getByText(/alterações não salvas|unsaved changes|cambios no guardados/i).first(),
...
test.describe('UX — landing sticky CTA + overflow', () => {
  ...
  test('home sticky CTA is wired with signup + safe-area chrome', async ({
    ...
    const padBottom = await sticky.evaluate((el) =>
...
test.describe('UX — bottom nav touch targets', () => {
  ...
  test('bottom nav links meet min height', async ({ page }) => {
    ...
    expect(moreBox?.height ?? 0).toBeGreaterThanOrEqual(40);
  });
});
